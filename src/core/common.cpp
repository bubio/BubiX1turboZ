...
int DLL_PREFIX my_swprintf_s(wchar_t *buffer, size_t sizeOfBuffer, const wchar_t *format, ...)
{
	va_list ap;
	va_start(ap, format);
	// win32 vswprintf() has no size argument, the C99/POSIX one does
#ifdef _WIN32
...
{
	/// Overrides the directory the core reads ROMs and settings from.
	/// Must be called before the VM is created; ignored on win32 and Qt.
	if(path != NULL) {
		my_strcpy_s(host_application_path, _MAX_PATH, path);
	} else {
		host_application_path[0] = _T('\0');
	}
}
#endif
...
#else
		// The host layer decides where ROMs and settings live (it follows the
		// platform conventions), so it must call set_application_path() before
		// the VM is created. Fall back to the current directory otherwise.
		if(host_application_path[0] == '\0') {
			if(getcwd(host_application_path, _MAX_PATH - 2) == NULL) {
				my_strcpy_s(host_application_path, _MAX_PATH, "./");
			}
		}
		my_strcpy_s(app_path, _MAX_PATH, host_application_path);
		size_t app_len = strlen(app_path);
		if(app_len > 0 && app_path[app_len - 1] != '/' && app_len < _MAX_PATH - 1) {
			app_path[app_len] = '/';
