...
// type definition
	...
	#endif
	// win32 virtual key codes, referenced by the VM keyboard devices
	#include "compat/vkcodes.h"
#endif
...
// max/min
	...
	// size_t is wider than unsigned int on LP64 hosts, so mixing sizeof() with
	// a 32bit value makes the overloads above ambiguous. These cover that case.
	// On ILP32 hosts size_t is unsigned int and the declarations above suffice.
	#if defined(SIZE_MAX) && defined(UINT_MAX) && (SIZE_MAX > UINT_MAX)
	#define CSP_HAS_WIDE_SIZE_T
	#endif
